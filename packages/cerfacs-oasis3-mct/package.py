...
class CerfacsOasis3Mct(MakefilePackage):
    ...
    maintainers("manodeep")

    # version("access-om2", branch="master", preferred=True)
    # version("access-esm1.5", branch="access-esm1.5")
    version("access-esm1.6", branch="OASIS3-MCT_5.0", preferred=True)
    variant("deterministic", default=False, description="Deterministic build.")
    variant("optimisation_report", default=False, description="Generate optimisation reports.")
    # variant("build_target", 
    #         default="static-libs-fortran",
    #         values=("static-libs", "shared-libs", "static-libs-fortran", "shared-libs-fortran", "static-and-shared-libs-fortran"),
    #         description="Build target for OASIS3-MCT libraries.")
    variant("static", default=True, description="Build static libraries.")
    # Setting to True as recommended by default by Spack docs (https://spack.readthedocs.io/en/latest/packaging_guide_creation.html#variant-names). 
    # Is not the default behaviour in ACCESS-NRI oasis3-mct (MS: 19 Aug 2025)
    variant("shared", default=False, description="Build shared libraries.")
    conflicts("+static +shared", msg="Cannot simultaneously build both shared and static libraries. Please specify exactly one of '+shared' or '+static'")
    ...
    __build_target = "static-libs-fortran"
    __lib_extn = ".a"
    # __valid_makefile_targets = ("static-libs", "shared-libs", "static-libs-fortran", "shared-libs-fortran", "static-and-shared-libs-fortran")
    ...
    __libs["psmile.MPI1"]["incfiles"] = [
        ...
        "mod_oasis_auxiliary_routines.mod",
    ...
    __libs["scrip"]["incfiles"] = [
        ...
        "remap_distance_gaussian_weight.mod",
        "remap_locc_weight.mod",
        "remap_vars.mod",
        "remap_write.mod",
        "timers.mod",
    ]
    ...
    def __init__(self, args):
        super().__init__(args)
        self.__lib_extn = '.a'
        self.__build_target = "static-libs-fortran"
        if '+shared' in self.spec:
            self.__lib_extn = '.so'
            self.__build_target = "shared-libs-fortran"
            self.__incdir = join_path(self.__builddir, "build-shared", "lib")

        for k in self.__libs.keys():
            self.__libs[k]["filename"] = "lib" + k + self.__lib_extn
    ...
    def url_for_version(self, version):
        allowed_versions = ("5.2", "5.1", "5.0")
        if version not in allowed_versions:
            raise ValueError("Invalid version: {0}. Allowed versions are: {1}".format(version, allowed_versions))

        return f"https://gitlab.com/cerfacs/oasis3-mct/-/archive/OASIS3-MCT_{version}/oasis3-mct-OASIS3-MCT_{version}.tar.gz"
...
Name: {k}
Description: OASIS3-MCT 5.0 {k} Library for Fortran
Version: 5.0
Libs: -L${{libdir}} -l{k}
...
"""
        ...
        # -ip is not supported by ifx
        config["oneapi"] = config["intel"].replace(" -ip", "")
...
"""
    ...
    def build(self, spec, prefix):
        ...
        with working_dir(join_path(self.stage.source_path, self.__makefiledir)):
            ...
            build("-f", "TopMakefileOasis3", f"{self.__build_target}")
